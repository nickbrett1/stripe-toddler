...
async fn handle_pos_inventory_barcode(req: Request, ctx: RouteContext<()>) -> Result<Response> {
...
async fn handle_terminal_connection_token(req: Request, ctx: RouteContext<()>) -> Result<Response> {
    if !validate_app_attest_auth(&req)? {
        return error_response("Unauthorized: Missing App Attest assertion", 401);
    }
    ...
    if let Some(secret) = stripe_json.get("secret").and_then(|s| s.as_str()) {
        ...
        error_response(
            &format!("Invalid Stripe Token Response: {}", stripe_text),
            502,
        )
    }
}

async fn handle_terminal_payment_intent(
    mut req: Request,
    ctx: RouteContext<()>,
) -> Result<Response> {
    if !validate_app_attest_auth(&req)? {
    ...
    match (intent_id, client_secret) {
        ...
        _ => error_response(
            &format!("Stripe payment intent creation failed: {}", stripe_text),
            502,
        ),
    }
}

async fn handle_terminal_capture(mut req: Request, ctx: RouteContext<()>) -> Result<Response> {
    if !validate_app_attest_auth(&req)? {
        return error_response("Unauthorized: Missing App Attest assertion", 401);
    }
    ...
    if status != "succeeded" {
        return error_response(
            &format!(
                "Stripe payment intent capture failed. Status: {}. Response: {}",
                status, stripe_text
            ),
            400,
        );
...
async fn handle_admin_inventory_post(mut req: Request, ctx: RouteContext<()>) -> Result<Response> {
    if !validate_admin_auth(&req, &ctx.env)? {
        return error_response("Unauthorized: Invalid Admin API Key", 401);
    }
...
async fn handle_admin_inventory_upload(
    mut req: Request,
    ctx: RouteContext<()>,
) -> Result<Response> {
    if !validate_admin_auth(&req, &ctx.env)? {
    ...
    let image_url = format!(
        "https://stripe-toddler-images.{}.r2.dev/{}",
        account_id, key
    );
    let resp = ImageUploadResponse { image_url, barcode };
    json_response(&resp)
}

async fn handle_admin_analytics(req: Request, ctx: RouteContext<()>) -> Result<Response> {
    if !validate_admin_auth(&req, &ctx.env)? {
        return error_response("Unauthorized: Invalid Admin API Key", 401);
    }
    ...
    let tx_stmt =
        db.prepare("SELECT * FROM transactions ORDER BY created_at DESC LIMIT ? OFFSET ?");
    ...
    for tx in tx_records {
        ...
        let api_items: Vec<LineItem> = items_records
            .into_iter()
            .map(|item| LineItem {
...
pub async fn main(req: Request, env: Env, _ctx: Context) -> Result<Response> {
    ...
    router
        ...
        .post_async("/api/attest/challenge", handle_attest_challenge)
        .post_async("/api/attest/verify", handle_attest_verify)
        // 2. POS Client endpoints
        .get_async("/api/pos/inventory/:barcode", handle_pos_inventory_barcode)
        .post_async(
            "/api/terminal/connection-token",
            handle_terminal_connection_token,
        )
        .post_async(
            "/api/terminal/payment-intent",
            handle_terminal_payment_intent,
        )
        .post_async("/api/terminal/capture", handle_terminal_capture)
        // 3. Admin Executive endpoints
        .get_async("/api/admin/inventory", handle_admin_inventory_get)
        .post_async("/api/admin/inventory", handle_admin_inventory_post)
        .post_async("/api/admin/inventory/upload", handle_admin_inventory_upload)
        .get_async("/api/admin/analytics", handle_admin_analytics)
        .run(req, env)
        .await
}
