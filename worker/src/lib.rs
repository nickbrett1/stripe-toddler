    ...
    router
        ...
        .get_async("/api/pos/inventory/:barcode", |req, ctx| async move {
            ...
            match item_str {
                ...
                None => {
                    // No hardcoded fallback items: an unknown barcode is a real
                    // 404 so the POS surfaces "Item Not Found" instead of
                    // silently serving a fake product that could mask failures.
