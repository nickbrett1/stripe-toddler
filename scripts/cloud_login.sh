...
if command -v tailscale &> /dev/null; then
  ...
  if ! sudo tailscale status &> /dev/null; then
    ...
    sudo tailscale up --hostname=stripe-toddler
  else
...
if command -v doppler &> /dev/null; then
  if doppler whoami &> /dev/null; then
    ...
    doppler setup --no-interactive --project stripe-toddler --config dev
  fi
else
...
if doppler run --project stripe-toddler --config dev -- env | grep -q "CLOUDFLARE_API_TOKEN"; then
  echo "✅ Found CLOUDFLARE_API_TOKEN in Doppler. Using token for authentication."
  # Verify connectivity
  if ! doppler run --project stripe-toddler --config dev -- npx wrangler whoami 2>&1 | grep -q "You are not authenticated"; then
...
doppler run --project stripe-toddler --config dev -- ./scripts/setup-wrangler-config.sh dev
