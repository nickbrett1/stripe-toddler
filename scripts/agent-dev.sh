...
# "stripe-toddler-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "stripe-toddler-dev" is overwritten by
...
PROJECT_NAME="stripe-toddler"
AGENT_NAME="stripe-toddler-dev"
WORKSPACE="/workspaces/stripe-toddler"
...
if [ -x "/workspaces/stripe-toddler/scripts/agent-dev.sh" ]; then
  "/workspaces/stripe-toddler/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
